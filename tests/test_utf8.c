...
static int test_utf8_from_wchar(void)
{
    wchar_t wbuf[32];
    char buf[64];
    size_t n;
    ...
    wbuf[1] = L'i';
    wbuf[2] = L'\0';
    n = utf8_from_wchar(wbuf, 2, buf, sizeof(buf));
    TEST(n == 2);
    ...
    wbuf[2] = 0x0438; /* и */
    wbuf[3] = L'\0';
    n = utf8_from_wchar(wbuf, 3, buf, sizeof(buf));
    TEST(n == 6);
...
static int test_utf8_to_wchar(void)
{
    char buf[64];
    wchar_t wbuf[32];
    size_t n;
    ...
    /* ASCII */
    memcpy(buf, "Hi", 3);
    n = utf8_to_wchar(buf, 2, wbuf, sizeof(wbuf) / sizeof(wchar_t));
    TEST(n == 2);
    ...
    buf[6] = '\0';
    n = utf8_to_wchar(buf, 6, wbuf, sizeof(wbuf) / sizeof(wchar_t));
    TEST(n == 3);
...
static int test_roundtrip(void)
{
    ...
    n = utf8_to_wchar(utf8, utf8Len, wbuf, 64);
    TEST(n != UTF8_UNKNOWN);
    wbuf[n] = L'\0';

    n = utf8_from_wchar(wbuf, n, buf, sizeof(buf));
    TEST(n != UTF8_UNKNOWN);
    buf[n] = '\0';
